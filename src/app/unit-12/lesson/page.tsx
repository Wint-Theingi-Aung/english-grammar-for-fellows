...
export default function Unit12LessonPage() {
  const data = getLessonsData(UNIT);
  const [needNounLesson, needToV1Lesson] = data.lessons;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10 lg:py-12">
      ...
      {/* Need + Noun */}
      <section className="bg-surface rounded-2xl border border-border p-6 mb-6 shadow-sm" aria-labelledby="need-noun-heading">
        <h2 id="need-noun-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
          ...
          {needNounLesson.title}
        </h2>
        <p className="text-ink-light mb-4 leading-relaxed">{needNounLesson.content}</p>
        {needNounLesson.details && (
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full text-sm border-collapse min-w-[480px]">
              ...
              <tbody>
                {needNounLesson.details.map((d, i) => (
      ...
      {/* Need to + V1 */}
      <section className="bg-surface rounded-2xl border border-border p-6 mb-8 shadow-sm" aria-labelledby="need-to-v1-heading">
        <h2 id="need-to-v1-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
          ...
          {needToV1Lesson.title}
        </h2>
        <p className="text-ink-light mb-4 leading-relaxed">{needToV1Lesson.content}</p>
        {needToV1Lesson.details && (
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full text-sm border-collapse min-w-[480px]">
              <thead>
                <tr>
              ...
              <tbody>
                {needToV1Lesson.details.map((d, i) => (
                  <tr key={i} className="border-b border-border last:border-b-0 hover:bg-surface-alt/50 transition-colors duration-150">
                    <td className="px-4 py-3 font-semibold text-ink">{d.concept ?? ""}</td>
                    <td className="px-4 py-3">
                      <span className="text-ink block">{d.example ?? ""}</span>
