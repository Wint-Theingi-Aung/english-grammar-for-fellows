...
export default function Unit11LessonPage() {
  const data = getLessonsData(UNIT);
  const [responsibilityLesson, notRequiredLesson, tensesLesson] = data.lessons;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10 lg:py-12">
      ...
      <section className="bg-surface rounded-2xl border border-border p-6 mb-6 shadow-sm" aria-labelledby="responsibility-heading">
        <h2 id="responsibility-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
          ...
          {responsibilityLesson.title}
        </h2>
        <p className="text-ink-light mb-4 leading-relaxed">{responsibilityLesson.content}</p>
        {responsibilityLesson.details && (
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full text-sm border-collapse min-w-[480px]">
              ...
              <tbody>
                {responsibilityLesson.details.map((d, i) => (
      ...
      <section className="bg-surface rounded-2xl border border-border p-6 mb-6 shadow-sm" aria-labelledby="notrequired-heading">
        <h2 id="notrequired-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
          ...
          {notRequiredLesson.title}
        </h2>
        <p className="text-ink-light mb-4 leading-relaxed">{notRequiredLesson.content}</p>
        {notRequiredLesson.details && (
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full text-sm border-collapse min-w-[480px]">
              <thead>
                <tr>
              ...
              <tbody>
                {notRequiredLesson.details.map((d, i) => (
                  <tr key={i} className="border-b border-border last:border-b-0 hover:bg-surface-alt/50 transition-colors duration-150">
                    <td className="px-4 py-3 font-semibold text-ink">{d.concept ?? ""}</td>
                    <td className="px-4 py-3">
                      <span className="text-ink block">{d.example ?? ""}</span>
      ...
      <section className="bg-surface rounded-2xl border border-border p-6 mb-8 shadow-sm" aria-labelledby="tenses-heading">
        <h2 id="tenses-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
        ...
        <p className="text-ink-light mb-4 leading-relaxed">{tensesLesson.content}</p>
        {tensesLesson.details && (
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full text-sm border-collapse min-w-[480px]">
              <thead>
                <tr>
