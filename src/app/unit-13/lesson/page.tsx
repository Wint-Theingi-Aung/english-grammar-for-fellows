...
export default function Unit13LessonPage() {
  const data = getLessonsData(UNIT);
  const [seemAdjLesson, seemToV1Lesson] = data.lessons;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10 lg:py-12">
      ...
      {/* Seem + Adjective */}
      <section className="bg-surface rounded-2xl border border-border p-6 mb-6 shadow-sm" aria-labelledby="seem-adj-heading">
        <h2 id="seem-adj-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
        ...
        <p className="text-ink-light mb-4 leading-relaxed">{seemAdjLesson.content}</p>
        {seemAdjLesson.details && (
      ...
      {/* Seem to + V1 */}
      <section className="bg-surface rounded-2xl border border-border p-6 mb-8 shadow-sm" aria-labelledby="seem-to-v1-heading">
        <h2 id="seem-to-v1-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
          ...
          {seemToV1Lesson.title}
        </h2>
        <p className="text-ink-light mb-4 leading-relaxed">{seemToV1Lesson.content}</p>
        {seemToV1Lesson.details && (
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full text-sm border-collapse min-w-[480px]">
              <thead>
                <tr>
              ...
              <tbody>
                {seemToV1Lesson.details.map((d, i) => (
                  <tr key={i} className="border-b border-border last:border-b-0 hover:bg-surface-alt/50 transition-colors duration-150">
                    <td className="px-4 py-3 font-semibold text-ink">{d.concept ?? ""}</td>
                    <td className="px-4 py-3">
                      <span className="text-ink block">{d.example ?? ""}</span>
