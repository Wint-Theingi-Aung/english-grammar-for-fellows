...
export default function Unit7LessonPage() {
  const data = getLessonsData(UNIT);
  const [shouldLesson, oughtLesson, comparisonLesson] = data.lessons;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10 lg:py-12">
      ...
      {/* Should for Advice and Expectations */}
      <section className="bg-surface rounded-2xl border border-border p-6 mb-6 shadow-sm" aria-labelledby="should-heading">
        <h2 id="should-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
        ...
        <p className="text-ink-light mb-4 leading-relaxed">{shouldLesson.content}</p>
        {shouldLesson.details && (
      ...
      {/* Ought to for Obligation and Duty */}
      <section className="bg-surface rounded-2xl border border-border p-6 mb-6 shadow-sm" aria-labelledby="ought-heading">
        <h2 id="ought-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
        ...
        <p className="text-ink-light mb-4 leading-relaxed">{oughtLesson.content}</p>
        {oughtLesson.details && (
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full text-sm border-collapse min-w-[480px]">
              <thead>
      ...
      {/* Should vs Ought to */}
      <section className="bg-surface rounded-2xl border border-border p-6 mb-8 shadow-sm" aria-labelledby="comparison-heading">
        <h2 id="comparison-heading" className="text-xl font-bold text-ink mb-3 flex items-center gap-2">
          ...
          {comparisonLesson.title}
        </h2>
        <p className="text-ink-light mb-4 leading-relaxed">{comparisonLesson.content}</p>
        {comparisonLesson.details && (
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full text-sm border-collapse min-w-[480px]">
              <thead>
                <tr>
              ...
              <tbody>
                {comparisonLesson.details.map((d, i) => (
                  <tr key={i} className="border-b border-border last:border-b-0 hover:bg-surface-alt/50 transition-colors duration-150">
                    <td className="px-4 py-3 font-semibold text-ink">{d.concept ?? ""}</td>
                    <td className="px-4 py-3">
                      <span className="text-ink block">{d.example ?? ""}</span>
