...
/**
 * Offline SQL export for seeding Units 12–16.
 * Generates a standalone SQL file that can be applied to any PostgreSQL database
 * without connecting during generation.
 *
 * Run: npx tsx scripts/export-seed-sql-unit12-16.ts
 * Output: scripts/unit-12-to-16-seed.sql
 */
...
(() => {
  ...
  const UNITS = [12, 13, 14, 15, 16];
  const DATA_DIR = path.join(__dirname, "..", "data");
  const OUTPUT_PATH = path.join(__dirname, "unit-12-to-16-seed.sql");
  ...
  function generateSql(unitResults: Record<number, any>): { sql: string; stats: { unitInserts: number; lessonInserts: number; exerciseInserts: number; titles: string[] } } {
    ...
    lines.push("-- ==========================================================================");
    lines.push("-- English Grammar for Fellows — Units 12–16 Seed Data");
    lines.push("-- Generated offline by scripts/export-seed-sql-unit12-16.ts");
  ...
  function main() {
    console.log("Validating JSON data files for Units 12–16...\n");
    ...
    for (let i = 0; i < stats.titles.length; i++) {
      console.log(`    Unit ${i + 12}: ${stats.titles[i]}`);
    }
    console.log(`\n  Total INSERT statements: ${stats.unitInserts + stats.lessonInserts + stats.exerciseInserts}`);
    console.log("\nDone. Apply with: psql $DATABASE_URL -f scripts/unit-12-to-16-seed.sql");
  }
