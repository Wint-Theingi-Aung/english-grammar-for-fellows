...
/**
 * Offline SQL export for seeding Units 35–37.
 * Generates a standalone SQL file that can be applied to any PostgreSQL database
 * without connecting during generation.
 *
 * Run: npx tsx scripts/export-seed-sql-unit35-37.ts
 * Output: scripts/unit-35-to-37-seed.sql
 */
...
(() => {
  ...
  const UNITS = [35, 36, 37];
  const DATA_DIR = path.join(__dirname, "..", "data");
  const OUTPUT_PATH = path.join(__dirname, "unit-35-to-37-seed.sql");
   ...
   * Used for simple text values only — NOT for JSON.
   */
  ...
  /**
   * Serialize a JS value to a PostgreSQL dollar-quoted JSON literal: $$...$$::jsonb
   * The value is JSON.stringify'd, then wrapped in $$ delimiters.
   * If the JSON itself contains $$, we double the dollar signs inside.
   */
  function dollarJson(val: unknown): string {
    const json = JSON.stringify(val);
    // Validate round-trip: parse what we just stringified to catch any issues
    JSON.parse(json);
    // If the JSON contains $$, escape by splitting into segments
    if (json.includes("$$")) {
      // Replace $$ with $$$$ inside the JSON body so PostgreSQL sees literal $$
  ...
  function generateSql(unitResults: Record<number, any>): { sql: string; stats: { unitInserts: number; lessonInserts: number; exerciseInserts: number; titles: string[] } } {
    ...
    lines.push("-- ==========================================================================");
    lines.push("-- English Grammar for Fellows — Units 35–37 Seed Data");
    lines.push("-- Generated offline by scripts/export-seed-sql-unit35-37.ts");
    ...
    for (const unitNum of UNITS) {
      ...
      for (let li = 0; li < lessonsData.lessons.length; li++) {
        ...
        const dataJsonDollar = dollarJson(dataObj);
        lines.push(
          `INSERT INTO lessons (id, unit_id, slug, title, content, data, sort_order) ` +
          `VALUES (${esc(lid)}, (SELECT id FROM units WHERE unit_number = ${unitNum}), ${esc(l.id)}, ${esc(l.title)}, ${esc(l.content)}, ${dataJsonDollar}, ${li + 1}) ` +
      ...
      for (let ei = 0; ei < exercisesData.exercises.length; ei++) {
        ...
        const questionsJsonDollar = dollarJson(ex.questions);
        lines.push(
          `INSERT INTO exercises (id, unit_id, slug, type, instructions, questions) ` +
          `VALUES (${esc(eid)}, (SELECT id FROM units WHERE unit_number = ${unitNum}), ${esc(ex.id)}, ${esc(ex.type)}, ${esc(ex.instructions)}, ${questionsJsonDollar}) ` +
  ...
  function main() {
    console.log("Validating JSON data files for Units 35–37...\n");
    ...
    for (let i = 0; i < stats.titles.length; i++) {
      console.log(`    Unit ${i + 35}: ${stats.titles[i]}`);
    }
    console.log(`\n  Total INSERT statements: ${stats.unitInserts + stats.lessonInserts + stats.exerciseInserts}`);
    console.log("\nDone. Apply with: psql $DATABASE_URL -f scripts/unit-35-to-37-seed.sql");
  }
