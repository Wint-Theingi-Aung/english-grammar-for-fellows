...
/**
 * Offline SQL export for seeding Units 41–43.
 * Generates a standalone SQL file that can be applied to any PostgreSQL database
 * without connecting during generation.
 *
 * Run: npx tsx scripts/export-seed-sql-unit41-43.ts
 * Output: scripts/unit-41-to-43-seed.sql
 */
...
(() => {
  ...
  const UNITS = [41, 42, 43];
  const DATA_DIR = path.join(__dirname, "..", "data");
  const OUTPUT_PATH = path.join(__dirname, "unit-41-to-43-seed.sql");
  ...
  function generateSql(unitResults: Record<number, any>): { sql: string; stats: { unitInserts: number; lessonInserts: number; exerciseInserts: number; titles: string[] } } {
    ...
    lines.push("-- ==========================================================================");
    lines.push("-- English Grammar for Fellows — Units 41–43 Seed Data");
    lines.push("-- Generated offline by scripts/export-seed-sql-unit41-43.ts");
  ...
  function main() {
    console.log("Validating JSON data files for Units 41–43...\n");
    ...
    for (let i = 0; i < stats.titles.length; i++) {
      console.log(`    Unit ${i + 41}: ${stats.titles[i]}`);
    }
    console.log(`\n  Total INSERT statements: ${stats.unitInserts + stats.lessonInserts + stats.exerciseInserts}`);
    console.log("\nDone. Apply with: psql $DATABASE_URL -f scripts/unit-41-to-43-seed.sql");
  }
