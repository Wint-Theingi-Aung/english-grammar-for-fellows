...
/**
 * Offline SQL export for seeding Units 31–34.
 * Generates a standalone SQL file that can be applied to any PostgreSQL database
 * without connecting during generation.
 *
 * Run: npx tsx scripts/export-seed-sql-unit31-34.ts
 * Output: scripts/unit-31-to-34-seed.sql
 */
...
(() => {
  ...
  const UNITS = [31, 32, 33, 34];
  const DATA_DIR = path.join(__dirname, "..", "data");
  const OUTPUT_PATH = path.join(__dirname, "unit-31-to-34-seed.sql");
  ...
  function generateSql(unitResults: Record<number, any>): { sql: string; stats: { unitInserts: number; lessonInserts: number; exerciseInserts: number; titles: string[] } } {
    ...
    lines.push("-- ==========================================================================");
    lines.push("-- English Grammar for Fellows — Units 31–34 Seed Data");
    lines.push("-- Generated offline by scripts/export-seed-sql-unit31-34.ts");
  ...
  function main() {
    console.log("Validating JSON data files for Units 31–34...\n");
    ...
    for (let i = 0; i < stats.titles.length; i++) {
      console.log(`    Unit ${i + 31}: ${stats.titles[i]}`);
    }
    console.log(`\n  Total INSERT statements: ${stats.unitInserts + stats.lessonInserts + stats.exerciseInserts}`);
    console.log("\nDone. Apply with: psql $DATABASE_URL -f scripts/unit-31-to-34-seed.sql");
  }
