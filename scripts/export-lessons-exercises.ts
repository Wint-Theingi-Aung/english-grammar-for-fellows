...
/**
 * Offline SQL export for seeding Lessons + Exercises for Units 1–5.
 * Generates a standalone SQL file that can be applied to any PostgreSQL database
 * without connecting during generation.
 *
 * This script assumes the `units` table already has rows for units 1–5.
 * It only INSERTs into `lessons` and `exercises` tables.
 *
 * Run: npx tsx scripts/export-lessons-exercises.ts
 * Output: scripts/lessons-exercises-unit-1-to-5.sql
 */
...
(() => {
  ...
  const OUTPUT_PATH = path.join(__dirname, "lessons-exercises-unit-1-to-5.sql");
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function generateSql(unitResults: Record<number, any>): { sql: string; stats: { lessonInserts: number; exerciseInserts: number; questionCount: number; titles: string[] } } {
    const lines: string[] = [];
    let lessonInserts = 0;
    ...
    lines.push("-- ==========================================================================");
    lines.push("-- English Grammar for Fellows — Lessons + Exercises (Units 1–5)");
    lines.push("-- Generated offline by scripts/export-lessons-exercises.ts");
    ...
    return { sql: lines.join("\n"), stats: { lessonInserts, exerciseInserts, questionCount, titles } };
  }
  ...
  function main() {
    ...
    console.log(`  Total questions validated:   ${stats.questionCount}`);
    ...
    console.log(`\n  Total INSERT statements: ${stats.lessonInserts + stats.exerciseInserts}`);
    console.log("\nDone. Apply with: psql $DATABASE_URL -f scripts/lessons-exercises-unit-1-to-5.sql");
  }
